...
“通用”设置控制打开视频时是否自动提取并生成，并通过紧凑的 Radio Chip Group 选择全局视频总结预设。选择“自定义”可展开 Prompt 编辑器；保存后再次打开设置默认折叠。“偏好字幕获取及总结语言”默认中文，用于首次匹配当前视频的字幕轨道；摘要页仍可通过“字幕”胶囊下拉切换该视频的具体轨道。
...
所有运行时 Prompt（摘要、问答、生图和连通性测试）统一由 `defaultPrompts.v2.ts` 管理。图片设置中可选择信息图、封面图、海报、插画、极简、Pixel RPG 或自定义生图风格，并可选择 `16:9`、`4:3`、`1:1`、`9:16`、`9:21` 比例；生图缓存会按风格 Prompt 指纹和图片比例隔离。设置页的选择控件使用面板内 listbox，选项固定在当前字段下方展开，避免 macOS 原生 `<select>` 菜单覆盖已选值。

### ChatGPT 网页生图（实验）

1. 在 ChatGPT 中新建一个专用 Project。
2. 在图片设置中选择“ChatGPT 网页生图”，填入 Project 根页的完整 URL。
3. 保持该 Project 根页打开，完成连通性测试后即可从视频摘要中发起生图；任务完成后会自动返回根页。
