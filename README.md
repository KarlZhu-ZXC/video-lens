...
Bilibili 视频页 userscript：读取视频信息和字幕，调用 OpenAI-compatible 文本模型生成流式摘要、视频洞察问答和一图流 JSON，再可选调用图片模型生成背景图，最后在前端合成中文信息图并导出 PNG。
...
构建产物在 `dist/video-summary.user.js`，安装到 Tampermonkey 后打开 Bilibili 视频页使用。
...
- 设置页支持真实 API 连通性测试；已保存 API Key 不会回填到密码输入框，留空保存会继续使用旧 key
- 设置页支持界面语言和字幕/总结语言；英文总结会优先尝试英文字幕并使用英文 prompt
- 摘要页显示字幕语言、字幕源选择器、缓存生成时间，并提供单视频缓存清理和全量缓存清理
- 所有 Tab 面板宽度通过侧边拖拽调整并持久化
- HTTPS 资源 URL 规范化，减少 HTTPS 页面中的 Mixed Content warning
- SPA 路由监听带 history wrapper 标记，降低重复 patch 风险

V1 不包含登录、付费、自建后端、云端历史记录、评论总结、多平台支持或 Chrome Extension 版本。
