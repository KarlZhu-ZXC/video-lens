...
import { buildOneImagePrompt, runOnePagePipeline } from '../src/onePage/onePagePipeline';
...
import { imageCache } from '../src/store/imageCache';
...
describe('one page regeneration', () => {
  it('builds the image request from the fixed userscript prompt and text summary', () => {
    expect(buildOneImagePrompt('核心总结')).toBe(
      '根据以下视频内容总结，生成一张信息可视化的精美配图，风格清晰美观，适合作为视频总结的封面图：\n\n核心总结',
    );
  });

  it('sends the fixed prompt directly to the image model', async () => {
    ...
    const imageAiClient = {
      async generateImage(request: { prompt: string }) {
        ...
        return { dataUrl: 'data:image/png;base64,dGVzdA==' };
      },
    };
    ...
    expect(requestPrompt).toBe(buildOneImagePrompt('摘要'));
    expect(result.generatedImage.dataUrl).toBe('data:image/png;base64,dGVzdA==');
  });

  it('reuses cached images and bypasses the cache when regeneration is forced', async () => {
    const data = new Map<string, string>();
    Object.defineProperty(globalThis, 'localStorage', {
      configurable: true,
      value: {
    ...
    const imageAiClient = {
      async generateImage() {
        calls += 1;
        return { dataUrl: `data:image/png;base64,${calls}` };
      },
    };

    await runOnePagePipeline({ summary, imageAiClient, config: DEFAULT_CONFIG });
    await runOnePagePipeline({ summary, imageAiClient, config: DEFAULT_CONFIG });
    const forced = await runOnePagePipeline({ summary, imageAiClient, config: DEFAULT_CONFIG, force: true });

    expect(forced.generatedImage.dataUrl).toBe('data:image/png;base64,2');
    expect(calls).toBe(2);
    imageCache.clear();
  });
});
