...
**其他选项**：界面语言、字幕获取策略、偏好字幕获取及总结语言、总结风格预设、图片风格预设、图片比例、面板宽度、YouTube 字幕来源、是否启用自动摘要等。
...
- 对少数字幕下载接口返回空内容的视频，脚本会尝试复用 YouTube 播放器自身成功获取的字幕响应，并读取当前页面的官方 Transcript 面板；如果逐句转写仍不可用，会明确提示字幕不可读，不再用章节时间线冒充完整转写。
